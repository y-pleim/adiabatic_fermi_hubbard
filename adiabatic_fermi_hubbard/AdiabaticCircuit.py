...
class AdiabaticCircuit():
    ...
    def evolution_operator(self,k):
        ...
        final_ham = self.hubbard_hamiltonian.jw_hamiltonian() #get SparsePauliOp representation of HubbardHamiltonian
        ...
        for i in range(len(final_ham_paulis)):
            circ.append(self.pauli_string_rotation(final_ham_paulis[i],self.time_step*k*delta_s*np.real(final_ham_coeffs[i])),seq)
    ...
    def run_eigensolver_comparison(self):
        ...
        result = gs_solver.solve(problem).eigenvalues[0] #extracts ground state energy as float
